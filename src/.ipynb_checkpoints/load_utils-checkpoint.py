...
if __name__ == '__main__':
    ...
    model = XLMWithLMHeadModel.from_pretrained("xlm-mlm-xnli15-1024")
    model = model.to(device)

    input_ids = torch.tensor([tokenizer.encode("Wikipedia was used to")], device=cuda0)
    language_id = tokenizer.lang2id["en"]
    langs = torch.tensor([language_id] * input_ids.shape[1], device=cuda0)
    ...
    ffn_activation, ffn_output, outputs = model(input_ids, langs=langs)
    print(ffn_activation, ffn_output)