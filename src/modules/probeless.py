...
def _get_overall_ranking(mean_vectors):
    ...
    return np.argsort(overall)[::-1].tolist()

def _get_overall_difference(mean_vectors):
    differ = np.zeros_like(mean_vectors[0])
    combins = list(itertools.combinations(mean_vectors, 2))
    for couple in combins:
        differ += np.abs(couple[0] - couple[1])
    return differ / len(combins)
...
def _get_tag_wise_ranking(mean_vectors_by_label, tag):
    ...
    if len(mean_vectors_by_label) > 1:
        summation = summation / (len(mean_vectors_by_label) - 1)
...
def get_neuron_ordering(X_train, y_train):
    ...
    return _get_overall_ranking(avg_embeddings)

def get_neuron_difference(X_train, y_train):
    avg_embeddings, average_embeddings_by_label = _get_mean_vectors(X_train, y_train)
    return _get_overall_difference(avg_embeddings)
...
def get_neuron_ordering_for_all_tags(X_train, y_train, idx2label):
    ...
    for c, qz in average_embeddings_by_label.items():
        ...
        summation_per_tag[idx2label[c]] = summation

    overall_ranking = np.argsort(overall)[::-1].tolist()
    overall_summation = overall / len(average_embeddings_by_label)
